*Analysis run with stata 17.0
*script for analysis of NfL by Nirosen Vijiaratnam Aug 2021
*load dataset with import excel "\\ad.ucl.ac.uk\homej\skgtvij\DesktopSettings\Desktop\Proband\ProbandNflanalaysissheet.xlsx", sheet("Sheet1") firstrow clear


*cleaning data, exclude outlier restrict dataset to high prob PD cases
replace Nfl_meanconc=500 if Nfl_meanconc>500
drop if change_diag_ind
drop if prob_of_pd_last <90

*generate log NfL and standardized log NfL 
generate Nfl_meanconc=log( Nfl_meanconc)
generate lnNfl_meanconc=log( Nfl_meanconc)

*binary logistic regression of normal progression versus fast progression (odd group) and AUC estimation and comparison of AUC
...
recode oddgroup 1=0 2=1 3=1 4=1 5=1 6=1 7=1 8=1
...
*Harrel C extimation and comparison between different variables, removed from final publication 
estat concordance
...
*youden and youdenmax calculation
